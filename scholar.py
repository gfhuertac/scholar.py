...
# Import Browser Cookie -- try 3 first, fall back to older
try:
    ...
    try:
        ...
        print('We need browser_cookie, sorry...')
        sys.exit(1)
...
class ScholarArticle(object):
    ...
    def as_dict(self):
        return dict([(key, self.attrs[key][0]) for key in self.attrs.keys()])
    ...
    def as_json(self, ensure_ascii=True, indent=None, separators=None, sort_keys=False):
        ...
        """
        import json
        return json.dumps(as_dict(self), ensure_ascii=ensure_ascii, indent=indent, separators=separators, sort_keys=sort_keys)
    ...
    def _parse_globals(self):
        ...
        if tag is not None:
            ...
            if raw_text is not None and len(raw_text) > 0:
                try:
                    try:
                        matchObj = re.search(r'([\d,]+) result', raw_text[0])
                        num_results = matchObj.group(1)
...
class ScholarQuery(object):
    ...
    def __init__(self):
        ...
        # The starting page requested from Scholar
        self.start = None
...
class CitesScholarQuery(ScholarQuery):
    ...
    def set_article(self, article):
        ...
        """
        msg = 'article ID must be numeric'
        self.article = ScholarUtils.ensure_int(article, msg)

    def get_url(self):
        if self.article is None:
            raise QueryArgumentError('cites query needs article ID')

        urlargs = {'article': self.article }
        ...
        urlargs['num'] = ('&num=%d' % self.num_results
                          if self.num_results is not None else '')
        urlargs['start'] = ('&num=%d' % self.start
...
class ScholarQuerier(object):
    ...
    def __init__(self):
        ...
        if ScholarConf.COOKIE_JAR_BROWSER:
            try:
                if ScholarConf.COOKIE_JAR_BROWSER == 'chrome':
                    self.cjar = browser_cookie.chrome()
                elif ScholarConf.COOKIE_JAR_BROWSER == 'firefox':
                    self.cjar = browser_cookie.firefox()
            except Exception as msg:
                ScholarUtils.log('warn', 'could not load cookies file: %s' % msg)
                self.cjar = MozillaCookieJar() # Just to be safe
